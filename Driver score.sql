-- drop table test_ric.driver_metrics_scaling_oct_1 

create  table test_ric.driver_metrics_scaling_oct_1  as
-- create  table test_ric.driver_metrics_scaling_oct_3_5  as
with all_taxi_drivers as
( 
SELECT 
meterid,
usertype,
...
and usertype IN (2)
-- and _id='68c27328822881066f92f731'

)
-- select usertype, count(*), count(distinct ref_driver_id) from all_taxi_drivers  group by 1;-- 26,604
, jrny_base as 
(
select 
...
and date ( (journey_created_at::timestamp AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Dubai') <  current_date-1
)
-- select count(*), count(distinct journey_id) from jrny_base where ref_driver_id is not null and vehicle_cat ='Taxi'  and journey_status in (9,10);1,295,244
...
-- select 
-- jrny_cnt_with_rating, count(distinct ref_Driver_id)as driver_cnt

-- from(
-- select ref_Driver_id, count (distinct case when rating is not null then journey_id end )  as jrny_cnt_with_rating
-- ,count(distinct journey_id ) as overall_jrny_cnt
-- from final_base  
-- where ref_driver_id is not null and vehicle_cat ='Taxi' 
-- group by 1 
-- ) group by 1;
...
, driver_summary_30d AS (
  ...
  FROM final_base
  WHERE local_created_date >= current_date-2 - INTERVAL '32 days'
  GROUP BY ref_driver_id
)

,driver_summary_zone_peak AS (
  SELECT 
    ref_driver_id,
    pickup_zone_name,
    peak,
    COUNT(DISTINCT journey_id) AS total_jrny_request_zone_peak,
    COUNT(DISTINCT CASE WHEN journey_status IN (9,10) THEN journey_id END) AS completed_jrny_cnt_zone_peak,
    COUNT(DISTINCT CASE WHEN journey_status = 14 THEN journey_id END) AS driver_cancelled_jrny_cnt_zone_peak,
    
    AVG(rating) AS avg_rating_zone_peak,
    COUNT(DISTINCT CASE WHEN ontime_jrny = 1 and journey_status IN (9,10) THEN journey_id END) AS on_time_jrny_cnt_zone_peak,
    COUNT(DISTINCT CASE WHEN cancel_time_sec < 30 AND journey_status = 14 THEN journey_id END) AS jrny_cancelled_in_less_than_30_sec_by_driver_zone_peak
  FROM final_base
  GROUP BY 1,2,3
)
,combined_data as 
...
-- LEFT JOIN driver_summary_zone_peak zp ON d.ref_driver_id = zp.ref_driver_id
)
...
-- select count(*), count(distinct ref_Driver_id) from summary where total_journeys >=5;
...
-- select * from scoring where rc_missing =0 and ar_rc is null;
, params AS 
(
  SELECT
...
, logistic_raw AS (
  SELECT f.*,
    ...
    -- lifetime
--    1.0 / (1.0 + EXP( -p.k_rt * ( GREATEST(LEAST(f.ar_lt,5.0),1.0) - p.m_rt ) )) AS rating_log_lt_raw,
-- -- recency
--     1.0 / (1.0 + EXP( -p.k_rt * ( GREATEST(LEAST(f.ar_rc,5.0),1.0) - p.m_rt ) )) AS rating_log_rc_raw
...
-- B) exclude flagged rows from defining scaling windows
...
-- select distinct rt_lt_scaled from scaled  where total_journeys <5;
