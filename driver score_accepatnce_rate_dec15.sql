...
(ref_driver_id,total_journeys,total_journeys_30d,completion_rate_pct_lt,driver_cancel_rate_pct_lt,avg_rating_lt,
 completion_rate_pct_30d,driver_cancel_rate_pct_30d,avg_rating_30d,cr_lt_scaled,dc_lt_scaled,rt_lt_scaled,
 cr_rc_scaled,dc_rc_scaled,rt_rc_scaled,lifetime_score,recency_score,final_score,query_run_timestamp,
 acceptance_rate_pct_lt,acceptance_rate_pct_30d,acc_lt_scaled,acc_rc_scaled
 )
...
-- adding acceptance rate metric
, jrny_history_acc AS (
  SELECT
    distinct 
    journey_id,
    driver_id AS ref_driver_id,
    driver_action,
...
, acceptance_30d AS (
  ...
  WHERE local_action_ts >= (SELECT start_30day_period FROM week_ref)
    AND local_action_ts <  (SELECT latest_sunday_end   FROM week_ref)
  GROUP BY 1
)
...
SELECT 
  d.ref_driver_id,
    ...
    COALESCE(al.total_journey_accepted_lt, 0) AS total_journey_accepted_lt,
    COALESCE(al.total_offered_jrny_lt,   0)   AS total_offered_jrny_lt,
    /* 30d counts */
    ...
    COALESCE(a30.total_journey_accepted_30d, 0) AS total_journey_accepted_30d,
    COALESCE(a30.total_offered_jrny_30d,   0)   AS total_offered_jrny_30d 
...
LEFT JOIN acceptance_lifetime al ON d.ref_driver_id = al.ref_driver_id
LEFT JOIN acceptance_30d      a30 ON d.ref_driver_id = a30.ref_driver_id
...
SELECT
  ...
  total_offered_jrny_lt,
  total_offered_jrny_30d,

  ROUND(100.0 * total_journey_accepted_lt  / NULLIF(total_offered_jrny_lt,0), 5)  AS acceptance_rate_pct,
  ROUND(100.0 * total_journey_accepted_30d / NULLIF(total_offered_jrny_30d,0), 5) AS acceptance_rate_30d_pct
...
, scoring AS (
  SELECT
    ...
    , COALESCE(s.acceptance_rate_pct, 0)       AS acc_lt
    , COALESCE(s.acceptance_rate_30d_pct, 0)   AS acc_rc

    , s.total_offered_jrny_lt
    , s.total_offered_jrny_30d

    , CASE WHEN COALESCE(s.total_offered_jrny_lt,0) < 5 THEN 1 ELSE 0 END AS acc_lt_small
    , CASE WHEN COALESCE(s.total_offered_jrny_30d,0) = 0 THEN 1 ELSE 0 END AS acc_rc_missing
  
  FROM summary s
)
-- select * from scoring where ref_Driver_id in ('6720bfbc5c1726bb1236d1ac');
...
(
  SELECT
    ...
    0.50::float AS m_acc, 7.0::float AS k_acc--acceptance_rate
)
, logistic_raw AS (
  SELECT f.*,
...
-- rating logistic (recency)
    ...
    1.0/(1.0+EXP(-p.k_acc*(GREATEST(LEAST(f.acc_lt/100.0,0.999999),0.000001)-p.m_acc))) AS acc_log_lt_raw,
    1.0/(1.0+EXP(-p.k_acc*(GREATEST(LEAST(f.acc_rc/100.0,0.999999),0.000001)-p.m_acc))) AS acc_log_rc_raw


  FROM scoring f
  CROSS JOIN params p
)
-- select acc_log_lt_raw, count(distinct ref_Driver_id) from logistic_raw group by 1;
...
, logistic_for_window AS (
  SELECT r.*,
        ...
        --  CASE WHEN lt_small=0 THEN acc_log_lt_raw END AS acc_log_lt_win,
        --  CASE WHEN rc_missing=0 THEN acc_log_rc_raw END AS acc_log_rc_win

          CASE WHEN acc_lt_small=0 THEN acc_log_lt_raw END AS acc_log_lt_win,
          CASE WHEN acc_rc_missing=0 THEN acc_log_rc_raw END AS acc_log_rc_win
...
, mm AS (
  SELECT
    ...
    MIN(acc_log_lt_win) AS min_acc_lt,      MAX(acc_log_lt_win) AS max_acc_lt,
    MIN(acc_log_rc_win) AS min_acc_rc,      MAX(acc_log_rc_win) AS max_acc_rc
...
, scaled AS (
 ...
 CASE
    WHEN acc_lt_small=1 THEN 1
    ELSE (w.acc_log_lt_raw - m.min_acc_lt) / NULLIF(m.max_acc_lt - m.min_acc_lt,0)
  END AS acc_lt_scaled,

 CASE
    WHEN acc_rc_missing=1 OR acc_lt_small=1 THEN NULL
    ELSE (w.acc_log_rc_raw - m.min_acc_rc) / NULLIF(m.max_acc_rc - m.min_acc_rc,0)
  END AS acc_rc_scaled
...
, scored AS (
  SELECT
    ...
    (0.35 * cr_lt_scaled)  +
    (0.10 * dc_lt_scaled)  +
    (0.15 * rt_lt_scaled)  AS lifetime_score,

    (0.40 * acc_rc_scaled) +
    (0.35 * cr_rc_scaled)  +
    (0.10 * dc_rc_scaled)  +
    (0.15 * rt_rc_scaled)  AS recency_score_raw
  FROM scaled sc
)
...
SELECT
 ref_driver_id,total_journeys,total_journeys_30d,completion_rate_pct_lt,driver_cancel_rate_pct_lt,avg_rating_lt,
 completion_rate_pct_30d,driver_cancel_rate_pct_30d,avg_rating_30d,cr_lt_scaled,dc_lt_scaled,rt_lt_scaled,
 cr_rc_scaled,dc_rc_scaled,rt_rc_scaled,lifetime_score,recency_score,final_score,

 (getdate() AT TIME ZONE 'Asia/Dubai')::timestamptz AS query_run_timestamp,
 acceptance_rate_pct_lt,acceptance_rate_pct_30d,acc_lt_scaled,acc_rc_scaled
 from
 (
...
SELECT
  ...
  ROUND( 0.40 * lifetime_score
    ...
    acc_lt AS acceptance_rate_pct_lt,
    acc_rc AS acceptance_rate_pct_30d,
